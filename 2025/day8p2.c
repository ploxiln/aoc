// USAGE: day8p2 [max-connections] < input.txt
#include <assert.h>
#include <ctype.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
		printf("USAGE: day8p2 [max-connections] < input.txt\n");
		return 2;
	}
	...
	for (size_t i = 0; i < shortest_len; i++) {
		// each point stores which circuit it is part of
		// (look ma, no hash maps! just indexes!)
		...
		if (circuits[ca] == (int)points_len || circuits[cb] == (int)points_len) {
			Point a = points[ia], b = points[ib];
			printf("DONE after %zu links\n", i+1);
			printf("Last link: (%d,%d,%d) <-> (%d,%d,%d)\n", a.x, a.y, a.z, b.x, b.y, b.z);
			printf("X-prod = %d\n", a.x * b.x);
			break;
		}
	}
